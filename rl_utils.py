...
def compute_advantage(gamma, lmbda, td_delta):
    """
    广义优势估计-->优势估计/回报估计：
    | 方法（简称）                     | 是否有 Critic | 是否用 Bootstrap | 偏差-方差特性           | 典型算法                    |
    | -------------------------- | ---------- | ------------- | ----------------- | ----------------------- |
    | **GAE**（你已给出）              | ✅          | ✅             | 可调 λ              | PPO、A3C、SAC             |
    | **TD(0) 残差优势**             | ✅          | ✅             | 高偏差、极低方差          | DDPG、TD3                |
    | **n-step TD 优势**           | ✅          | ✅             | 偏差随 n 减小、方差随 n 增大 | Rainbow                 |
    | **蒙特卡洛回报减基线**              | ✅          | ❌             | 零偏差、高方差           | REINFORCE with baseline |
    | **蒙特卡洛回报（无基线）**            | ❌          | ❌             | 零偏差、极高方差          | Vanilla REINFORCE       |
    | **V-trace**（off-policy 修正） | ✅          | ✅             | 类似 GAE，但能修正 IS    | IMPALA                  |
    | **Retrace(λ)**（Q 版本修正）     | ✅          | ✅             | 低方差、低偏差           | ACER                    |
